...
app.post('/api/chatbot', async (req, res) => {
    try {
        ...
        // Simple local response system (always works)
        const localResponse = generateLocalResponse(message, context);

        // Try Gemini for enhanced response (optional)
        try {
            const GeminiService = require('./services/geminiService');
            const geminiResponse = await Promise.race([
                GeminiService.getChatResponse(message, context || {}),
                new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 5000))
            ]);

            if (geminiResponse && geminiResponse.success && geminiResponse.response) {
            ...
            console.log('⚠️ Gemini unavailable, using local response');
        }
        ...
        res.json({
            response: "I'm here to help! Ask me about irrigation, soil moisture, crop health, or NPK nutrients.",
            timestamp: new Date(),
            source: 'fallback'
        });
    }
...
try {
    ...
    // Router will handle /api/chatbot, but we already registered it above as fallback
    // This ensures route always works
    console.log('✅ Chatbot router loaded (fallback route also registered)');
} catch (error) {
    console.warn('⚠️ Chatbot router not available, using direct route:', error.message);
    console.log('✅ Chatbot fallback route registered');
}
...
app.post('/api/analyze-image', async (req, res) => {
    try {
        const { image, cropType, sensorData } = req.body;
        ...
        const analysis = await GeminiService.analyzeImage(base64Image, cropType || 'Unknown', sensorData);
