...
class GeminiService {
    constructor() {
        this.API_KEY = process.env.GEMINI_API || 'AIzaSyD3DPnnd54Kb0JPC6T6y1E82zLrXVF-elo';
        // Updated Gemini API endpoints - using gemini-1.5-flash-latest for v1beta compatibility
        this.IMAGE_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${this.API_KEY}`;
        this.CHAT_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${this.API_KEY}`;
    }
     ...
     */
    async analyzeImage(base64Image, cropType = 'Unknown', sensorData = null) {
        try {
...
Provide detailed response in JSON format:
{
    ...
    async generateChatResponse(userMessage, context) {
        try {
            const { predictions, sensorData, cropType, imageAnalysis } = context;

            // Build context prompt
            let contextPrompt = `You are an expert AI Agriculture Assistant. Provide helpful, accurate advice based on the following data:\n\n`;
            ...
            contextPrompt += `Provide a helpful, concise answer based on this data. If predictions show critical issues, emphasize urgency.`;
